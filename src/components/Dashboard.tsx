...
export function Dashboard() {
  ...
  const upCount = Object.values(stocks).filter((s) => s && s.change > 0).length;
  const downCount = Object.values(stocks).filter((s) => s && s.change < 0).length;
  const flatCount = Object.values(stocks).filter((s) => s && s.change === 0).length;

  return (
    <div className="space-y-2.5">
      {/* Stats + Filters */}
      <div className="flex flex-wrap items-center gap-2 text-[10px]">
        <div className="rounded border border-[var(--card-border)] bg-[var(--card-bg)] px-2 py-1">
          <span className="text-gray-500">企业</span>{" "}
          <span className="font-bold text-white">{supplyChainNodes.length}</span>
          <span className="text-gray-600 mx-1">|</span>
          <span className="text-gray-500">上市</span>{" "}
        ...
        {fetchedAt && (
          <span className="text-[9px] text-gray-600 ml-auto">
            {new Date(fetchedAt).toLocaleTimeString("zh-CN")} · 30s刷新
          </span>
        )}
      </div>
      ...
      <div className="flex flex-wrap gap-1">
        ...
        {ALL_TIERS.map((t) => {
          const count = supplyChainNodes.filter((n) => n.tier === t.key).length;
          return (
            <button
              key={t.key}
              onClick={() => setActiveTier(t.key)}
              className={`rounded px-2 py-1 text-[10px] font-medium transition-colors ${
                activeTier === t.key ? "text-white" : "text-gray-500 hover:text-gray-300"
              }`}
              style={
                activeTier === t.key
        ...
        return (
          <div key={tier}>
            <div className="flex items-center gap-1.5 mb-1">
              <div className="h-2.5 w-0.5 rounded-full" style={{ backgroundColor: cfg.color }} />
              <h2 className="text-[10px] font-bold" style={{ color: cfg.color }}>{cfg.label}</h2>
              <span className="text-[9px] text-gray-500 font-mono">
                {nodes.length} 家 · {tierTotal}%
              </span>
              <div className="flex-1 h-px bg-white/5" />
            </div>
            <div className="mb-1.5 rounded border border-white/5 bg-white/[0.02] px-2 py-1">
              <div className="flex h-4 w-full rounded overflow-hidden bg-white/5">
                {nodes.map((node, idx) => (
                  <div
                    key={node.id}
                    className="relative flex items-center justify-center first:rounded-l last:rounded-r"
                    style={{
                      width: `${(node.supplyShare / tierTotal) * 100}%`,
                      background: `linear-gradient(135deg, ${cfg.color}cc, ${cfg.color}88)`,
                      borderRight: idx < nodes.length - 1 ? "1px solid rgba(0,0,0,0.3)" : "none",
                    }}
                    title={`${node.name}: ${node.supplyShare}%`}
                  >
                    {node.supplyShare >= 15 && (
                      <span className="text-[8px] font-bold text-white/90 truncate px-0.5">{node.supplyShare}%</span>
                    )}
                  </div>
                ))}
              </div>
              <div className="mt-0.5 flex flex-wrap gap-x-3 gap-y-0">
                {nodes.map((node) => (
                  <div key={node.id} className="flex items-center gap-0.5">
                    ...
                    />
                    <span className="text-[9px] text-gray-400">{node.name}</span>
                    <span className="text-[9px] font-mono font-semibold" style={{ color: cfg.color }}>
                      {node.supplyShare}%
                    </span>
                  </div>
                ))}
